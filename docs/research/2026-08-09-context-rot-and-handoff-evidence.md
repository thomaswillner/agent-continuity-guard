...
The initial v1 50% checkpoint, 65% transition, and 75% refusal policy, together with its absolute 80K, 120K, and 160K ceilings, must therefore be labeled `PROVISIONAL_UNCALIBRATED`. The evidence directly contradicts treating 80K as a universally early safe boundary: NoLiMa found severe degradation at 32K, and some conditions degrade at still shorter lengths.[8] It contradicts treating 120K as reliably usable for multi-session work: LongMemEval found 30% to 60% declines at approximately 115K tokens relative to oracle evidence-only context.[9] It provides no basis for treating 160K as a quality ceiling. Conversely, first-party Gemini documentation reports very high performance on some single-query retrieval cases at much longer context, so the absolute ceilings may be unnecessarily conservative for simple workloads.[17] Thresholds must be calibrated by model, harness, workload, tool-output profile, and transition mechanism.

**Operator policy decision, 2026-08-09.** The retired v1 seed above is replaced by `conservative-longrun-v2`: checkpoint at 25% rendered use or 16,000 cumulative tokens, require compact/handoff at 40% or 24,000, and refuse protected continuation at 50% or 32,000. This is a conservative operational choice, not a value prescribed by the papers. It deliberately checkpoints and transitions before the 32K failure region and treats 32K as a refusal ceiling. Exact status remains `PROVISIONAL_UNCALIBRATED`; update gates protect the selected default from silent upward drift, while profile-specific behavioral calibration remains necessary for any safety or efficiency claim.
...
The retired 50%/65%/75% policy was a reasonable initial experimental seed because it created space between observation, transition, and refusal, but it intervened too late for the difficult 32K cases. Operator-selected v2 uses 25%/40%/50% and 16K/24K/32K. It is still not empirically optimal. NoLiMa and RULER support earlier probes because failures occur by 32K.[6][8] Gemini single-query retrieval and some million-token models show that the new defaults may be unnecessarily expensive for simple workloads.[17] Calibration may select an explicit profile, but an update cannot silently raise the built-in v2 default.
...
**Default-update gate.** Pass requires exact generation-2 source and installed-artifact defaults at 25%/40%/50% and 16K/24K/32K, an append-only v1-to-v2 migration decision, and fail-closed behavior for a missing or silently raised built-in value. This proves policy drift resistance, not empirical safety.
