...
CREATE TABLE IF NOT EXISTS sensitive_local_values (
...
CREATE TRIGGER IF NOT EXISTS sensitive_local_values_no_update
BEFORE UPDATE ON sensitive_local_values
BEGIN
    SELECT RAISE(ABORT, 'sensitive local values are immutable');
END;

CREATE TRIGGER IF NOT EXISTS sensitive_local_values_no_delete
